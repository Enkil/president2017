...
//    $mail->Port = $settings['EmailSMTPPort'];
    $mail->CharSet = 'UTF-8';
    $mail->setFrom($settings['EmailFromAddr'],$settings['EmailFromName']);
...
//    $cellFeed->editCell(1,4, "age"); // 1st row, 4th column
    ...
    $row = array('дата'=>$today, 'статус'=>'Новая с телефоном', 'телефон'=>$phone, 'e-mail'=>$email, 'фио'=>$name, 'промо-код'=>$promo, 'адрес доставки'=>$city, 'источник'=>($utm_source) ? $utm_source : '');
    $listFeed->insert($row);
